...
export const fetchJobs=(city,niche,searchKeyword)=>async()=>{
try{
...
export const postJob=(data)=>async(dispatch)=>{
    ...
    try {
      const response = await axios.post(
        'http://localhost:4000/api/v1/job/post',data,
        { withCredentials: true,headers:{"Content-Type":"application/json"} }
      );
      dispatch(jobSlice.actions.successForSingleJob(response.data.message));
      dispatch(jobSlice.actions.clearAllErrors());
    } catch (error) {
      dispatch(jobSlice.actions.failureForSingleJob(error.response.data.message));
    }
